...
if __name__ == "__main__":
    ...
    for i in range(4):
      path = check_list[i]
      model = DistillBart().load_from_checkpoint(checkpoint_path=f"drive/MyDrive/mlbart_ckpt/{check_list[i]}").model
      print("model_loaded")
      ...
      for s1,s2,lang_code in dataset:
          ...
          gen_token=model.generate(**inputs, forced_bos_token_id=tokenizer.lang_code_to_id[lang_code])
